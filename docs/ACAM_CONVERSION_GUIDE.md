...
### Required Inputs
- **--acam-file**: Path to ACAM pickle file containing cell geometry data
...
To load ACAM pickled tissue files without requiring the original ACAM source code, `scripts/tissue_acam_to_vertex.py` uses a `StubUnpickler`. This replaces missing classes with generic objects, allowing you to access basic data (attributes, arrays) even if the original class definitions are unavailable.
